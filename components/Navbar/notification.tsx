          ...
          <div className="divide-y divide-gray-200 max-h-96 overflow-y-auto">
            {unReadNotifications && unReadNotifications.length > 0 ? (
              unReadNotifications.map((notification: NotificationInterface) => (
                <Link href={ notification.type === 'addProposal' ? `/adminDashboard/proposals/${notification.typeId}` : `/freelancer/proposals/${notification.typeId} `} key={notification._id} className="p-4 hover:bg-gray-50" onClick={() => handleLinkClick(notification._id)}>
